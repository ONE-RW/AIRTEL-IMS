...
$exeSource = Join-Path $PSScriptRoot "dist\AirtelIMSDeviceAgent.exe"
...
if (Test-Path $exeSource) {
  Copy-Item $exeSource (Join-Path $InstallDir "AirtelIMSDeviceAgent.exe") -Force
} else {
...
try {
  if (Test-Path (Join-Path $InstallDir "AirtelIMSDeviceAgent.exe")) {
    $taskAction = New-ScheduledTaskAction -Execute (Join-Path $InstallDir "AirtelIMSDeviceAgent.exe")
  } else {
  ...
  Register-ScheduledTask `
    -TaskName "Airtel IMS Device Agent" `
    -Action $taskAction `
    -Trigger $taskTrigger `
    -Settings $taskSettings `
    -Principal $taskPrincipal `
    -Description "Uploads workstation health telemetry to Airtel IMS." `
    -Force | Out-Null
}
finally {
  Pop-Location
...
Write-Host "Update $targetConfig with the real asset tag and API settings before the next boot."
Write-Host "You can test immediately with:"
if (Test-Path (Join-Path $InstallDir "AirtelIMSDeviceAgent.exe")) {
  Write-Host "  $InstallDir\AirtelIMSDeviceAgent.exe --once"
} else {
